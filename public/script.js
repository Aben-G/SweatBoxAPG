...
function initializeScrollEffects() {
    // Parallax effect for hero section - DISABLED to prevent layering issues
    const heroSection = document.querySelector('.about-hero-enhanced');
    if (heroSection) {
        // Remove parallax effect that causes sections to go under each other
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (membershipForm) {
        membershipForm.addEventListener('submit', async function(e) {
            ...
            const submitBtn = this.querySelector('.registration-submit-btn');
            ...
            try {
                ...
                if (response.ok) {
                    showNotification('Registration submitted successfully! We\'ll contact you soon to complete the process.', 'success');
                    this.reset();
                } else {
                    showNotification(result.error || 'Failed to submit registration. Please try again.', 'error');
                }
            } catch (error) {
                console.error('Error:', error);
                showNotification('Network error. Please check your connection and try again.', 'error');
